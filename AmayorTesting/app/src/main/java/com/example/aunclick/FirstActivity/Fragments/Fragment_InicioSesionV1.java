...
public class Fragment_InicioSesionV1 extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        String savedTipoUsuario = prefs.getString("tipoUsuario", "");
        if (!savedUsuario.isEmpty()) {
            nombreUsuarioLogin.setText(savedUsuario);
        }
        if (!savedTipoUsuario.isEmpty()) {
            for (int i = 0; i < usuarios.length; i++) {
                if (usuarios[i].equals(savedTipoUsuario)) {
                    tipoUsuarioLogin.setSelection(i);
                    break;
                }
            }
        }
                ...
                case "Cliente":
                    ...
                    if (clienteId != -1) {
                        ...
                        if (mantenerSesionIniciada.isChecked()) {
                            ...
                            editor.putInt("userId", clienteId);
                            editor.putString("userType", "Cliente");
                            editor.apply();
                        } else {
                            editor.clear().apply();
                        }
                ...
                case "Tienda":
                    ...
                    if (tienda != null) {
                        ...
                        if (mantenerSesionIniciada.isChecked()) {
                            editor.putString("usuario", usuario);
                            editor.putString("tipoUsuario", tipoUsuario);
                            editor.putInt("userId", tienda.getIdPersona());
                            editor.putString("userType", "Tienda");
                            editor.apply();
                        } else {
                            editor.clear().apply();
                        }
                ...
                case "Repartidor":
                    ...
                    if (repartidorId != -1) {
                        ...
                        if (mantenerSesionIniciada.isChecked()) {
                            editor.putString("usuario", usuario);
                            editor.putString("tipoUsuario", tipoUsuario);
                            editor.putInt("userId", repartidorId);
                            editor.putString("userType", "Repartidor");
                            editor.apply();
                        } else {
                            editor.clear().apply();
                        }
